...
- 使用文本转语音、参考音频克隆或极致克隆。
- 在稳定混合模式下复用参考音频输入缓存。
- 普通模式合成一个 WAV；批量模式按非空行输出多个 WAV，不限制任务数量，每完成一条立即导出且可随时中止。
- 刷新或重新打开页面时会热接管后端正在运行的任务，同步推理参数、进度和最新输出，不会重新提交任务。
- 每条任务可选择不切分、按句子数量切分，或按目标字数累计到完整句末后切分。
- 为参考音频、原始结果和后处理结果提供页面内试听。
- 批量输出增量加入会话列表，不会中断列表中正在试听的音频；可将满意条目标记进 WAV 元数据。
- 当前会话常驻最新 50 条；超出范围但仍在播放的条目会保留到暂停或播放结束。
- 将满意结果直接移动到所选训练集。
...
入口：`http://127.0.0.1:8810/datasets`

- 使用系统文件夹选择器载入训练集，显示音频、同名 LAB 文本和实时统计。
- 待审核音频按需渲染高频红、低频蓝的频谱图，便于快速识别破音、宽频杂音和异常底噪。
- 可按 WAV 元数据中的 marked 状态筛选全部、已标记或未标记音频。
- 点击“保留”确认样本；点击“删除”会同时删除真实音频与同名 LAB。
- 对带有完整声音设计元数据的音频，可保持模型、LoRA、推理和切分参数不变，仅更换 Seed 重做并原位替换。
- 自动轮播会在队列更新后播放最顶部的音频，适合连续人工筛选。
- 快速去重按 LAB 全文精确匹配，每组保留文件名排序最前的一条并删除其余重复训练对。
- 审核进度按训练集持久化；“重新审核全部”只会清除确认记录，不会删除文件。
