...
def run():
    ...
    num_workers = int(getattr(hps.train, "num_workers", 1))
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        generator_model = net_g.module if hasattr(net_g, "module") else net_g
        if generator_model.use_noise_scaled_mas:
            current_mas_noise_scale = (
                generator_model.mas_noise_scale_initial
                - generator_model.noise_scale_delta * global_step
            )
            generator_model.current_mas_noise_scale = max(current_mas_noise_scale, 0.0)
        ...
        if rank == 0:
            ...
            should_save = (global_step + 1) % hps.train.eval_interval == 0
            should_save = should_save or (epoch == hps.train.epochs and batch_idx + 1 == len(train_loader))
            if should_save:
