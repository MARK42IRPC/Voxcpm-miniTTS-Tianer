...
def distribute_phone(n_phone, n_word):
    ...
    for _ in range(n_phone):
        minimum = min(phones_per_word)
        phones_per_word[phones_per_word.index(minimum)] += 1
...
def g2p(text, pad_start_end=True, tokenized=None):
    ...
    for group in ph_groups:
        ...
        if w.upper() in eng_dict:
            ...
            # The input is already a single tokenizer word. Calling G2p.__call__
            # here needlessly requires NLTK sentence tokenization and POS data.
            phone_list = list(filter(lambda p: p != " ", _g2p.predict(w.lower())))
            for ph in phone_list:
                if ph in arpa:
                    ph, tn = refine_ph(ph)
                    phones.append(ph)
